...
BOOST_AUTO_TEST_CASE( test_iomem_add ) {
   ...
   BOOST_CHECK_NO_THROW( add_iomem_region( 0, MAX_PHYS_ADDR, "replaced - all") ) ;
   BOOST_CHECK( validate_iomem() ) ;

   BOOST_CHECK_EQUAL( get_iomem_region_description(         0x000 ), "replaced - all" ) ;
   BOOST_CHECK_EQUAL( get_iomem_region_description(         0x100 ), "replaced - all" ) ;
...
BOOST_AUTO_TEST_CASE( test_iomem_add_bad ) {
   ...
   BOOST_CHECK_FAIL( add_iomem_region( 0x000, 0x001, "" ) ) ;
